...
func transport5GSMMessage(ue *context.AmfUe, anType models.AccessType,
	...
	err = client.Set("IP", time1, 0).Err()
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("Allocation IP start time is", time1)
...
func HandleRegistrationRequest(ue *context.AmfUe, anType models.AccessType, procedureCode int64,
	...
	pong, err := client.Ping().Result()
	fmt.Println(pong, err)
	err = client.Set("Authentication", time1, 0).Err()
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("Authentication start time is", time1)
...
func HandleInitialRegistration(ue *context.AmfUe, anType models.AccessType) error {
	...
	//--------------set time for Initial Registration----------------//
	...
	pong, err := client.Ping().Result()
	fmt.Println(pong, err)
	err = client.Set("Registration", time1, 0).Err()
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println("Initial Registration start time is", time1)
...
func HandleRegistrationComplete(ue *context.AmfUe, accessType models.AccessType,
	...
	val, err := client.Get("Registration").Result()
	if err != nil {
		fmt.Println(err)
	}
	...
	fmt.Println("Initial Registration end time is :", time2)
	time_total := time2 - time1

	fmt.Println("Latency for Initial Registration is : ", time_total)

	pong, err := client.Ping().Result()
	fmt.Println(pong, err)

	client.LPush("Reglatency", time_total)
	fmt.Println("Latency for Registration is stored")

	count, err := client.LLen("Reglatency").Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(count)
	if count >= 10 {
		fmt.Println("Process of printing average latency for Allocate IP")
		...
		latency_total = 0
		for i <= count {
			nrf[i], err = client.LPop("Reglatency").Result()
			if err != nil {
				panic(err)
			}
		...
		client.LPush("Reglatency", latency_avg)
		fmt.Println("Registration latency average is stored")
	}
