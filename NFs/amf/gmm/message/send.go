...
func SendSecurityModeCommand(ue *context.RanUe, eapSuccess bool, eapMessage string) {
	...
	val, err := client.Get("Authentication").Result()
	if err != nil {
		fmt.Println(err)
	}
	...
	fmt.Println("Authentication end time is :", time2)
	time_total := time2 - time1

	fmt.Println("Latency for Authentication is : ", time_total)

	pong, err := client.Ping().Result()
	fmt.Println(pong, err)

	client.LPush("authlatency", time_total)
	fmt.Println("Latency for Authentication is stored")

	count, err := client.LLen("authlatency").Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(count)
	if count >= 10 {
		fmt.Println("Process of printing average latency for Authentication")
		...
		latency_total = 0
		for i <= count {
			nrf[i], err = client.LPop("authlatency").Result()
			if err != nil {
				panic(err)
			}
		...
		client.LPush("authlatency", latency_avg)
		fmt.Println("Authentication latency average is stored")
	}
