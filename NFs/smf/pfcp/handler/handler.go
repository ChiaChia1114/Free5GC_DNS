...
func HandlePfcpSessionDeletionResponse(msg *pfcpUdp.Message) {
	...
	val2, err := client.Get("IP").Result()
	if err != nil {
		fmt.Println(err)
	}

	time11, err := strconv.ParseInt(val2, 10, 64)
	if err != nil {
		fmt.Println(err)
	}
	time22 := time.Now().UnixNano() / 1e3
	fmt.Println("Allocation IP end time is :", time22)
	time_total := time22 - time11

	fmt.Println("Latency for Allocation IP is : ", time_total)

	pong, err := client.Ping().Result()
	fmt.Println(pong, err)

	client.LPush("IPlatency", time_total)
	fmt.Println("Latency for Allocate IP is stored")

	count, err := client.LLen("IPlatency").Result()
	if err != nil {
		panic(err)
	}
	fmt.Println(count)
	if count >= 10 {
		...
		latency_total = 0
		for i <= count {
			nrf[i], err = client.LPop("IPlatency").Result()
			if err != nil {
				panic(err)
			}
		...
		client.LPush("IPlatency", latency_avg)
		fmt.Println("Allocate IP latency average is stored")
	}
